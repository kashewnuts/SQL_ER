...
def wiley_corrected_version():
    ...
    配送日と注文日の差が3日以上の注文
    """
    return Orders.objects.select_related().filter(
...
def date_calculation_django():
    ...
    return Orders.objects.select_related().annotate(
        diff_days=F('orderreceipts__delivery_date') - F('order_date')
    ).filter(
        orderreceipts__delivery_date__gt=F('order_date') + timedelta(days=3)
    ).values('order_id', 'order_name', 'diff_days')
...
def helen_window_function():
    ...
    return Orders.objects.select_related().annotate(
        item_count=Window(
            expression=Count('orderreceipts'),
            partition_by=[F('order_id')]
        )
    ).distinct().values('order_id', 'order_name', 'order_date', 'item_count')
...
def complex_conditional_query():
    ...
    max_delay_subquery = OrderReceipts.objects.filter(
        ...
        max_delay=Max(F('delivery_date') - F('order__order_date'))
    )['max_delay']
    
    return Orders.objects.annotate(
        max_delay=Subquery(
            OrderReceipts.objects.filter(
                order_id=OuterRef('order_id')
            ).aggregate(
                max_delay=Max(F('delivery_date') - F('order__order_date'))
            ).values('max_delay')
...
def performance_optimized_query():
    ...
    """
    return Orders.objects.select_related().prefetch_related(
        'orderreceipts_set'
    ).annotate(
        item_count=Count('orderreceipts'),
        max_delivery_delay=Max(
            F('orderreceipts__delivery_date') - F('order_date')
        )
    ).filter(
        max_delivery_delay__gte=3
    ).values(
        'order_id', 'order_name', 'order_date', 
        'item_count', 'max_delivery_delay'
    )
