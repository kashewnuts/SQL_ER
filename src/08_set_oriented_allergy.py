...
from django.db.models import (
    Case, When, Value, Count, F, Q, 
    IntegerField, CharField, Window, Exists, OuterRef
)
...
def find_same_address_non_set():
    ...
    for addr_item in all_addresses:
        ...
        people = list(Addresses.objects.filter(address=address).values('name', 'family_id'))
        if len(people) > 1:
...
def find_same_address_set_oriented():
    ...
    集合指向アプローチ（推奨）
    """
    return Addresses.objects.values('address').annotate(
...
def same_address_details():
    ...
    return Addresses.objects.filter(
        address__in=Addresses.objects.values('address').annotate(
            cnt=Count('address')
        ).filter(cnt__gt=1).values('address')
...
def family_address_grouping():
    ...
    家族単位での住所集約
    """
    return Addresses.objects.values('family_id').annotate(
        family_size=Count('name'),
        addresses=Count('address', distinct=True),
        address_list=models.functions.Concat(
            'address',
...
def department_completion_set_oriented():
    ...
    集合指向による部署完了チェック（推奨）
    """
    return Departments.objects.values('department', 'division').annotate(
        total_tasks=Count('*'),
        completed_tasks=Count(Case(
            When(check_flag='Y', then=1),
...
def having_clause_example():
    ...
    """
    return Departments.objects.values('department').annotate(
        division_count=Count('division'),
        completed_divisions=Count(Case(
            When(check_flag='Y', then='division'),
            output_field=IntegerField()
        ))
    ).filter(
...
def set_difference_operation():
    ...
    completed_departments = Departments.objects.values('department', 'division').annotate(
        total=Count('*'),
        completed=Count(Case(When(check_flag='Y', then=1)))
    ).filter(total=F('completed')).values('department', 'division')
    
    # 差集合（未完了部署）
    completed_set = set(
        (d['department'], d['division']) for d in completed_departments
    )
...
def set_union_operation():
    ...
    return Departments.objects.filter(
        condition1 | condition2
    ).values('department', 'division').distinct()
