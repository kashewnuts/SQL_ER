...
from django.db.models import (
    F, Min, Window, OuterRef, Subquery
)
    ...
    return Receipts.objects.filter(
        seq__in=Subquery(
            Receipts.objects.filter(
                customer_id=OuterRef('customer_id')
            ).aggregate(min_seq=Min('seq'))['min_seq']
        )
...
def patient_subquery_with_join():
    ...
    """
    # サブクエリで最小枝番を取得
    min_seqs = Receipts.objects.values('customer_id').annotate(
        min_seq=Min('seq')
    )
...
def get_min_seq_per_customer():
    ...
    顧客ごとの最小枝番を取得
    """
    return Receipts.objects.values('customer_id').annotate(
        min_seq=Min('seq')
    )
