...
from django.db.models import (
    Case, When, Value, Sum, Q, IntegerField, CharField
)
...
def union_redundant_solution():
    ...
    # 31-60日前
    q2 = PizzaSales.objects.filter(
        sale_date__range=(base_date - timedelta(days=60), base_date - timedelta(days=31))
    ).values('customer_id').annotate(
    ...
    # 61-90日前
    q3 = PizzaSales.objects.filter(
        sale_date__range=(base_date - timedelta(days=90), base_date - timedelta(days=61))
    ).values('customer_id').annotate(
    ...
    # 91日以上前
    q4 = PizzaSales.objects.filter(
        sale_date__lt=base_date - timedelta(days=91)
    ).values('customer_id').annotate(
...
def case_improved_solution():
    ...
    return PizzaSales.objects.values('customer_id').annotate(
        term_0_30=Sum(
            Case(
                When(
                    sale_date__range=(base_date - timedelta(days=30), base_date),
        ...
        # 何らかの売上がある顧客のみ
        Q(term_0_30__gt=0) | Q(term_31_60__gt=0) | Q(term_61_90__gt=0) | Q(term_91_plus__gt=0)
...
def performance_comparison():
    ...
    return {
        'union_query': str(union_query.query),
        'case_query': str(case_query.query)
    }
