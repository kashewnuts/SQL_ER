...
from django.db.models import (
    Case, When, Value, Sum, F, Q, IntegerField, OuterRef, Subquery
)
...
def wiley_redundant_union():
    ...
    """
    # 出荷可能業者のパート
    shippable_suppliers = Suppliers.objects.filter(
        ship_flg='可'
    ).values('city').annotate(
        able_cnt=Sum('item_cnt')
    )
    
    # 出荷可能業者で条件を満たすもの
    q1 = Suppliers.objects.filter(
        ship_flg='可'
    ).filter(
        city__in=[s['city'] for s in shippable_suppliers]
    ).extra(
        where=["item_cnt >= (SELECT SUM(item_cnt) * 0.5 FROM suppliers WHERE ship_flg = '可' AND city = suppliers.city)"]
    ).values('sup', 'city', 'ship_flg', 'item_cnt')
    
    # 出荷不可能業者のパート
    non_shippable_suppliers = Suppliers.objects.filter(
        ship_flg='不可'
    ).values('city').annotate(
        disable_cnt=Sum('item_cnt')
    )
    
    # 出荷不可能業者で条件を満たすもの
    q2 = Suppliers.objects.filter(
        ship_flg='不可'
    ).filter(
        city__in=[s['city'] for s in non_shippable_suppliers]
    ).extra(
        where=["item_cnt >= (SELECT SUM(item_cnt) * 0.5 FROM suppliers WHERE ship_flg = '不可' AND city = suppliers.city)"]
    ).values('sup', 'city', 'ship_flg', 'item_cnt')
...
def helen_cte_alternative():
    ...
    city_totals = {
        item['city']: {
            'able_cnt': item['able_cnt'],
            'disable_cnt': item['disable_cnt']
        }
...
def robert_case_expression():
    ...
    # 辞書形式で保存
    city_lookup = {
        stat['city']: stat for stat in city_stats
    }
...
def django_cte_patterns():
    ...
    subquery_pattern = Suppliers.objects.filter(
        item_cnt__gte=Subquery(
            Suppliers.objects.filter(
                city=OuterRef('city'),
                ship_flg=OuterRef('ship_flg')
            ).aggregate(
                avg_cnt=Sum('item_cnt') * 0.5
            ).values('avg_cnt')
        )
    )
    
    # パターン2: annotateを使用した段階的構築
    annotate_pattern = Suppliers.objects.values('city', 'ship_flg').annotate(
        total_cnt=Sum('item_cnt'),
        threshold=F('total_cnt') * 0.5
...
def modern_django_approach():
    ...
    return Suppliers.objects.annotate(
        ...
        item_cnt__gte=F('required_threshold')
