...
from django.db.models import (
    Case, When, Value, Sum, F, IntegerField, Window
)
...
def window_function_cumulative():
    ...
    ).values('shop_id', 'sale_date', 'sales_amt', 'cumlative_amt').order_by('shop_id', 'sale_date')
...
def window_function_stock_trend():
    ...
    """
    return StockHistory.objects.annotate(
        prev_price=Window(
    ...
    ).values('ticker_symbol', 'sale_date', 'closing_price', 'prev_price', 'calculated_trend')
