...
def array_search_operations():
    ...
    if hasattr(EmpChildArray, 'children'):
        # ArrayFieldの場合のみ実行可能
        return EmpChildArray.objects.filter(
            children__contains=['タロウ']
        ).values('emp_id', 'emp_name', 'children')
    
    # 代替案：正規化されたテーブルでの検索
    return Employee.objects.filter(
        employeechild__child_name='タロウ'
...
def normalized_child_search():
    ...
    """
    return Employee.objects.filter(
        employeechild__child_name__in=['タロウ', 'ジロウ']
    ).distinct().annotate(
        child_count=Count('employeechild')
...
def element_list_search():
    ...
    要素リストでの検索（推奨）
    """
    return ListElement.objects.filter(
        element='A'
    ).values('list_id').distinct()
    ...
    def get_array_length(model_class, array_field, target_length):
        ...
        if 'postgresql' in connection.settings_dict['ENGINE']:
            ...
            return model_class.objects.extra(
                where=[f"array_length({array_field}, 1) >= %s"],
            ...
            return Employee.objects.annotate(
                child_count=Count('employeechild')
            ).filter(child_count__gte=target_length)
